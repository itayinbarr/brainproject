...
function Breadcrumb({ crumb, leaf, color }) {
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '3px 5px', marginBottom: 12 }}>
      ...
      <span style={{ fontSize: 11, fontWeight: 700, color: color }}>{leaf}</span>
    </div>
  );
}

function SelectionCard({ node, color, catLabel, description, related, onSelect, onFocus, onIsolate, onClose, isolated }) {
  if (!node) return null;
  const sideLabel = node.side === 'median' ? 'Midline' : (node.side === 'left' ? 'Left' : 'Right');
  return (
    <div className="glass glass-top-hi pop" key={node.id} style={{
      position: 'absolute', left: 358, bottom: 16, width: 372, maxWidth: 'min(372px, calc(100vw - 374px - 88px))',
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 10 }}>
        ...
        <div style={{ flex: 1 }}>
          <div className="eyebrow" style={{ color: color, marginBottom: 3 }}>{catLabel}</div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <button onClick={() => onFocus(node.id)} style={actBtn(true)}><Icon name="target" size={15} /> Focus</button>
        <button onClick={() => onIsolate(node.id)} style={actBtn(false, isolated)}><Icon name="isolate" size={15} /> {isolated ? 'Isolated' : 'Isolate'}</button>
      </div>
    </div>
  );
}
